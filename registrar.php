...
if 	((strlen(htmlspecialchars($_POST['nombre']) >= 1)) &&
	...
	(strlen(htmlspecialchars($_POST['contraseña']) >= 1)))
	{
		...
		{
		?>
			<h3 class="bad">¡El usuario ya esta registrado! <br>Ingrese otro usuario</h3>
		<?php	
		}
		else
		{
			if (isset($_FILES['foto']['name']))
			 {
				...
				if (in_array($fileextension, $allowedfileextensions)) {
					...
					estadocivil, foto, usuario, pass) VALUES ('$nombre','$apellido','$email','$fecha','$tipo','$documento','$telefono','$hijos','$estadocivil', '$binariosimagen', '$usuario','$pass')";	
